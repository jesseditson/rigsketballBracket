...
var save = module.exports = function(req,res,next){
  ...
  if(data.operation == 'saveBand'){
    ...
    async.parallel([function(done){
      if(score1 && data.band1position){
        db.bands.findOne({position : data.band1position},function(err,pBand){
          console.log('band 1 : ',err, pBand);
          if(err) return done();
          pBand.scores[data.round] = score1;
          delete pBand._id;
          console.log('setting',pBand,'for',data.band1position);
          db.bands.update({position : data.band1position},{$set : pBand},function(err){
            done();
          })
        });
      } else {
      ...
      if(score2 && data.band2position){
        db.bands.findOne({position : data.band2position},function(err,pBand){
          console.log('band 2 : ',err, pBand);
          if(err) return done();
          pBand.scores[data.round] = score2;
          delete pBand._id;
          console.log('setting',pBand,'for',data.band2position);
          db.bands.update({position : data.band2position},{$set : pBand},function(err){
            done();
          })
        });
      } else {
