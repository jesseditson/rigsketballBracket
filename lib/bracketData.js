...
var renderBrackets = module.exports = function(insertAllowed,callback){
  ...
  async.parallel({
    ...
    callback(null,{full : full, leftBrackets : leftBrackets, rightBrackets : rightBrackets, winnerBracket : winnerBracket, winningBand : winner, insertAllowed : insertAllowed})
  })
}
...
var generateBrackets = function(bands,rounds,scores){
  Object.keys(rounds).reverse().forEach(function(rNum,rIndex){
    ...
    Object.keys(round).forEach(function(pNum){
      ...
      if(band1 && band2){
        var winner = band1.scores[rNum] > band2.scores[rNum] ?
                    0 :
                    band2.scores[rNum] > band1.scores[rNum] ?
                    1 :
                    false
